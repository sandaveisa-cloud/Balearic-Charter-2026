...
export default function DestinationDetail({ destination }: DestinationDetailProps) {
  ...
  useEffect(() => {
    if (!destination.seasonal_data || Object.keys(destination.seasonal_data).length === 0) {
      const generate = async () => {
        ...
        try {
          const res = await fetch('/api/generate-destination-content', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ destinationName, locale, region: destination.region }),
          })
  ...
  }, [destination, locale, destinationName])
    ...
    <article className="min-h-screen bg-white">
      <header className="relative h-[50vh] md:h-[65vh] w-full overflow-hidden bg-luxury-blue">
        {optimizedImageUrl ? (
          ...
          <div className="absolute inset-0 bg-slate-200 flex items-center justify-center"><MapPin className="w-20 h-20 text-slate-400" /></div>
        )}
        ...
        <div className="absolute inset-x-0 bottom-0 pb-12">
          <div className="container mx-auto px-4">
            ...
            <Link href="/" className="inline-block px-8 py-3 bg-luxury-gold text-luxury-blue font-bold rounded-lg shadow-lg uppercase tracking-wider text-sm">
              {t('viewFleet')}
            </Link>
          </div>
        </div>
