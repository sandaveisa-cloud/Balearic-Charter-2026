...
export default function AdminDashboard() {
  ...
  const uploadImage = async (file: File, yachtId: string): Promise<string | null> => {
    try {
      const fileExt = file.name.split('.').pop()
      const fileName = `${yachtId}-${Date.now()}.${fileExt}`
  ...
  // Handle image upload for fleet - adds to gallery_images array
  const handleImageUpload = async (yachtId: string, file: File) => {
    ...
    try {
      const imageUrl = await uploadImage(file, yachtId)
      
      if (imageUrl) {
        // Get current yacht to append new image to gallery_images array
        ...
        const updatedImages = [...allImages, imageUrl]
        
        // Update fleet record - set both main_image_url (first image) and gallery_images
        const { error } = await supabase
          .from('fleet')
          .update({ 
            main_image_url: updatedImages[0] || imageUrl, // First image as main
        ...
        if (error) {
          ...
          setFleet(prev => prev.map(yacht => 
            yacht.id === yachtId ? { 
              ...yacht, 
              main_image_url: updatedImages[0] || imageUrl,
              gallery_images: updatedImages 
            } : yacht
          ))
          setFleetSuccess(prev => ({ ...prev, [yachtId]: 'Image uploaded successfully!' }))
          setTimeout(() => {
  ...
  const handleDestinationImageUpload = async (destinationId: string, file: File) => {
    ...
    try {
      const imageUrl = await uploadImage(file, destinationId)
  ...
  const handleCulinaryMediaUpload = async (culinaryId: string, file: File) => {
    ...
    try {
      const imageUrl = await uploadImage(file, culinaryId)
  ...
  const handleSettingsUpdate = async (updates: Record<string, string>) => {
    ...
    try {
      ...
      if (errors.length > 0) {
        console.error('[Admin] Errors updating site_settings:', errors)
        alert('Some settings failed to update')
      } else {
        console.log('[Admin] Site settings updated successfully in database:', results)
        // Update local state
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          {activeTab === 'overview' && (
            <div>
              <h2 className="text-2xl font-bold text-luxury-blue mb-6">Dashboard Overview</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-gray-50 p-6 rounded-lg">
                  <h3 className="text-lg font-semibold text-gray-700 mb-2">Booking Inquiries</h3>
                  <p className="text-3xl font-bold text-luxury-blue">{inquiries.length}</p>
                  <p className="text-sm text-gray-500 mt-2">Total inquiries</p>
                </div>
                <div className="bg-gray-50 p-6 rounded-lg">
                ...
                <div className="bg-gray-50 p-6 rounded-lg">
                  <h3 className="text-lg font-semibold text-gray-700 mb-2">Recent</h3>
                  <p className="text-3xl font-bold text-green-600">
                    {inquiries.filter(i => new Date(i.created_at) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length}
                  </p>
            ...
            <div>
              ...
              {settingsSuccess && (
                <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                  <p className="text-green-800">{settingsSuccess}</p>
                </div>
              )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
                ...
                <div key={index} className="relative group">
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 rounded">
                    <button
                      ...
                      disabled={index === 0}
                      className="p-1 bg-white rounded disabled:opacity-50 disabled:cursor-not-allowed"
                      title="Move up"
                    >
                  ...
                  {index === 0 && (
                    <div className="absolute top-1 left-1 bg-luxury-blue text-white text-xs px-2 py-1 rounded">
                      Main
                    </div>
                  )}
                </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <p className="text-sm text-gray-500">Max 5MB, JPG/PNG/WebP. First image is the main image.</p>
          </div>
        </div>
